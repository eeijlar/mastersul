...
int getint(char *);
int getbyte (char *);
int mf_w_sysex_event(unsigned long,unsigned char *,unsigned long);
int mf_w_midi_event(unsigned long , unsigned int ,unsigned int ,unsigned char * ,unsigned long);
int mf_w_meta_event(unsigned long,int,unsigned char*,unsigned long);

/** JNI Methods **/
char *convArray[256];
static char midArray[256];
static char textArray[256];
void t2mf(int,char *[]);