...
{
	if ( Mf_getc == NULLFUNC )
		printf("mfread() called without setting Mf_getc"); 
...
static readmt(char *s)		/* read through the "MThd" or "MTrk" header string * */
{
	int n = 0;
	char *p = s;
	int c;
...
static egetc()			/* read a single character and abort on EOF */
{
	int c = (*Mf_getc)();
...
static void readheader()		/* read a header chunk */
{
...
static readtrack()		 /* read a track chunk */
{
	...
	while ( Mf_toberead > 0 ) {
		...
		if ( sysexcontinue && c != 0xf7 )
			printf("didn't find expected continuation of a sysex");

		if ( (c & 0x80) == 0 ) {	 /* running status? */
			if ( status == 0 )
				printf("unexpected running status");
			running = 1;
			c1 = c;
			c = status;
		}
...
void mfwrite(int format,int ntracks,int division,FILE *fp) 
{
    ...
    if ( Mf_putc == NULLFUNC )
	    printf("mfmf_write() called without setting Mf_putc");

    if ( Mf_wtrack == NULLFUNC )
	    printf("mfmf_write() called without setting Mf_mf_writetrack"); 
...
void mf_w_track_chunk(int which_track,FILE *fp,int (*wtrack)())
...
printf("length = %d\n",(int) trklength);
 	...
 	if(fseek(fp,offset,0) < 0)
	    printf("error seeking during final stage of write");
 ...
 */
int mf_w_meta_event(unsigned long delta_time,int type,unsigned char* data,unsigned long size)
 ...
 */
int mf_w_sysex_event(unsigned long delta_time,unsigned char *data,unsigned long size)
{
    unsigned long i;
...
unsigned long mf_sec2ticks(int secs,unsigned int division,float tempo)
{    
