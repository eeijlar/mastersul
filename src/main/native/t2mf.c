...
Java_MainWindow_t2mf (JNIEnv *env, jobject j2, jstring text, jstring mid) 
{

	const jbyte *textFile;
    const jbyte *midiFile;
	textFile=(*env)->GetStringUTFChars(env,text, NULL);
	if(textFile==NULL) {
		return;
	}
	midiFile=(*env)->GetStringUTFChars(env,mid, NULL);
	if(midiFile==NULL) {
		return;
	}

	strcpy(textArray,textFile);
	strcpy(midArray,midiFile);

	convArray[0]="t2mf.exe";
	convArray[1]=textArray;
	convArray[2]=midArray;

	t2mf(arg,convArray);
	clearBuffer();
	(*env)->ReleaseStringUTFChars(env,text, textFile);
	(*env)->ReleaseStringUTFChars(env,mid, midiFile);
	
	return;	
}


void t2mf(int arg,char *convArray[])
{
   
    FILE *tfopen();
	int flg;
	arg_index=0;
	while (flg = crack (arg, convArray, "Rr", 0)) {
		switch (flg) {
		case 'r':
		case 'R':
			Mf_RunStat = 1;
    ...
    if ( arg_index < arg ) {
        yyin = tfopen (convArray[arg_index++], "r");
	}
    else {
        yyin = stdin;
	}
    if (arg_index < arg ) {
        F = tfopen(convArray[arg_index],"wb");
	}
    else {
#ifdef SETMODE
...
static int mywritetrack()
{
	...
	case EOF:
		...
		if (k!=denom) printf ("Illegal TimeSig");
		data[0] = nn;
		data[1] = i;
		data[2] = cc;
		data[3] = bb;
		...
		if (i<-7 || i>7)
		    printf ("Key Sig must be between -7 and 7");
...
static void checkchan()
{
    ...
    if (yyval < 1 || yyval > 16)
	printf ("Chan must be between 1 and 16");
    chan = yyval-1;
}
...
static void checknote()
{
    ...
    if (yyval < 0 || yyval > 127)
	printf ("Note must be between 0 and 127\n");
...
static void checkval()
{
    ...
    if (yyval < 0 || yyval > 127)
	printf ("Value must be between 0 and 127");
    data[1] = yyval;
}
...
static void splitval()
{
	...
	printf ("Value must be between 0 and 16383");
    data[0] = yyval%128;
    data[1] = yyval/128;
}
...
static void get16val()
{
	...
	printf ("Value must be between 0 and 65535");
    data[0] = (yyval>>8)&0xff;
    data[1] = yyval&0xff;
}
...
static void checkcon()
{
	...
	syntax();
    if (yyval < 0 || yyval > 127)
	printf ("Controller must be between 0 and 127");
    data[0] = yyval;
}
...
static void checkprog()
{
    ...
    if (yyval < 0 || yyval > 127)
	printf ("Program number must be between 0 and 127");
    data[0] = yyval;
}
...
static void gethex()
{
	...
	int i = 0;
	    ...
	    if (! buffer) printf ("Out of memory");
	}
	while (i<yyleng-1) {
	    c = yytext[i++];
rescan:
